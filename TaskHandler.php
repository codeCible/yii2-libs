 ...
 * @version v1.0 20171027 lihuixu@basicfinder.com;
 * @version v2 2018.5.25 lihuixu@basicfinder.com;
 * @version saas 2019.4.1 lihuixu@basicfinder.com
 * @version lite 2019.9.30 yuyongshi@basicfinder.com
 * 
 */
...
class TaskHandler
{
    ...
    public function fetchData($dataId = 0, $userId = 0, $dataCount = 2, $dataSort = 0)
    {
        ...
        //有源数据的情况, 比如:标注类,
        if ((int)$this->category['type'] == Category::TYPE_LABEL)
        {
            //---------------------------------------
            //找到可执行的分步
            //并检查该分步是否有数据
        ...
        //没有数据源的情况, 比如:采集类,
        elseif ((int)$this->category['type'] == Category::TYPE_COLLECTION)
        {
            //---------------------------------------
            //找到可执行的分步
            //并检查该分步是否有数据
            ...
            $dataIds = Data::find()
            ->select(['id'])
            ->where(['project_id' => $this->projectId, 'batch_id' => $this->batchId, 'name' => $this->userId])
            ->asArray()->column();
            $_logs['$dataIds'] = $dataIds;
            
            
            //找到未执行的数据
            $newDataIds = array_diff($dataIds, $didDataIds);
            ...
            if (!$newDataIds)
            {
                ...
                //增加统计数
                //Project::updateAllCounters(['data_count' => 1], ['id' => $this->projectId]);
            ...
            {
                Yii::info(__CLASS__.' '.__FUNCTION__.' '.__LINE__.' $newDataIds exist '.json_encode($_logs));
                
                if (count($newDataIds) > 1)
                {
                    Yii::error(__CLASS__.' '.__FUNCTION__.' '.__LINE__.' $newDataIds more than one '.json_encode($_logs));
                }
                
                reset($newDataIds);
                $nextItemId = current($newDataIds);
                $_logs['$nextItemId'] = $nextItemId;
            }
...
//             }
        ...
        elseif ((int)$this->category['type'] == Category::TYPE_EXTERNAL)
        {
            //---------------------------------------
            //找到可执行的分步
            //并检查该分步是否有数据
            ...
            elseif (!$hasData)
            {
                Yii::error(__CLASS__.' '.__FUNCTION__.' '.__LINE__.' task_fetchData_noDataRemain '.json_encode($_logs));
                return FormatHelper::result('', 'task_data_waitting', yii::t('app', 'task_data_waitting'));
            }
            //该分步有数据
            else
    ...
    public function fetch($dataId, $userId, $dataCount, $dataSort = 0)
    {
        ...
        foreach ($dataIds as $dataId)
        {
            ...
            //执行时是否加载原始数据结果, 只有无结果时才加载
            if (!empty($this->step['is_load_result']) && empty($workResult['result']))
            {
                if (!empty($dataResult['result']))
                {
                    if (is_array($dataResult['result']))
                    {
                        $workResult['result'] = json_encode($dataResult['result']);
                    }
                    else
                    {
                        $workResult['result'] = $dataResult['result'];
                    }
                }
                elseif (!empty($dataResult['ai_result']))
                {
                    if (is_array($dataResult['ai_result']))
                    {
                        $workResult['result'] = json_encode($dataResult['ai_result']);
                    }
                    else
                    {
                        $workResult['result'] = $dataResult['ai_result'];
                    }
                }
            }
    ...
    public function temporaryStorage($dataId, $userId, $dataId_, $workResult_)
    {
        ...
        //保存最终结果
        $attributes = [
            'result' => json_encode($workResult_)
        ];
        WorkResult::updateAll($attributes, ['work_id' => $work['id']]);
    ...
    public function auditEditSubmit($dataId, $userId, $workId_, $dataId_, $dataResult_)
    {
        ...
        //更新作业记录
        $attributes = [
            'result' => json_encode($dataResult_),
//             'stat' => json_encode($statUser_)
        ];
        WorkResult::updateAll($attributes, ['id' => $workResult['id']]);
